...
/**
 * Write=({bbs})=>{}
 * 매개변수로 전달 받은 변수중 나는 bbs만 받겠다
 *
 * App.js 에서 전달 받은 상태(bbsVO 를 bbs로 받음)는
 * 전달받아서 매개변수로 부터 추출하는 순간 상태의 기능을 상실한다
 * 고정된(Read only)형태의 props(properties)가 된다
 * props가 된 변수 객체는 보여주는 용도로는 사용할수있지만 값을 변경하는것은 불가능해진다
 * 값을 변경하려고 시도를 하면 오류가 발생한다
 *
 * 상태(변수,객체)는 반드시 선언된 컴포넌트에서만 변경이 가능하다
 * 입력은 Write.js에서 실행하지만 입력된 데이터를 전파하기 위해서는
 * App.js에게 상태를 변경해달라고 요청해야한다
 *
 * 이러한 기능을 해결하기위해서는 App.js에 상태를 변경하는 함수를 선언하고
 * 그 함수를 다시 하위(Write.js)컴포넌트에게 전달하여
 * 대리 실행하도록 코드를 작성해야한다
 *
 *
 * Write=({bbs,onBBsChagne})코드는
 * Write=(props)=>{
